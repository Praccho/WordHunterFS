...
function GameBoard({ dict }) {
    ...
    const handleMouseEnter = (rowIndex, colIndex) => {
        if (!dragging && (!selectedTile || (selectedTile.rowIndex !== rowIndex || selectedTile.colIndex !== colIndex))) {
            if (!(playing && tileLetters[rowIndex][colIndex] == "")) {
                ...
                for (let j = 0; j < numCols; j++) {
                    if (copyTileLetters[i][j] != "") {
                        ...
                        if (neighbors.length == 0 || Math.random() <= 0.1) {
                            ...
                            while (!(numTiles > 32) && placedLetterFreq[lttrInd] >= Math.min(2,Math.ceil(dist[lttrInd] / 0.04))) {
    ...
    function handleSignIn(user, pass) {
        ...
        axios.post(`${backendUrl}/login`, requestData)
            .then(response => {
                let copyUserData = {
    ...
    function handleRegister(user, pass) {
        ...
        axios.post(`${backendUrl}/register`, requestData)
            .then(response => {
    ...
    function handleUpdate(score) {
        ...
        if (score > userData.highscore) {
            userDataCopy.highscore = score;
        }
        userDataCopy.gamesPlayed += 1;

        let requestData = {
            "username": userDataCopy.username,
            "gamesPlayed": userDataCopy.gamesPlayed,
            "highscore": userDataCopy.highscore
        };

        axios.post(`${backendUrl}/update`, requestData)
            .then(response => {
    ...
    return (
        <div className="full-window">
            ...
            <SideBar
                ...
                timer={timeLeft}
                playing={playing}
                onSignIn={(u,p) => handleSignIn(u,p)}
                onRegister={(u,p) => handleRegister(u,p)}
                userData={userData}
            />
        </div>);
}
